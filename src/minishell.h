...
int				ft_find_quotes_in_line(t_data *data,
					char **matrix, int count_x);
...
void			collect_redirections(t_ast *node,
					t_ast **redirections, int *index);
...
int				handle_heredoc_result(int result, t_data *data, int pipefd[2],
					t_ast *node);
int				setup_heredoc_pipe(t_data *data, int original_stdin,
					int pipefd[2]);
